...
class Todo(BaseModel):
    ...
    priority: int = Field(gt=0, lt=6, description="priority must between 1 - 5")
    complete: bool
...
@app.get("/todos/user")
async def read_all_by_the_user(user: dict = Depends(get_current_user), db: Session =Depends(get_db)):
    if user is None:
