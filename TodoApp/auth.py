...
SECRET_KEY = "KlgH145KgD1vfbDeh5kN5v21eqo123lK"
ALGORITHM = "HS256"
...
def create_access_token(username: str, user_id: str, expired_delta: Optional[timedelta] = None):
...
async def login_for_access_token(form_date :OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    ...
    token = create_access_token(user.username, user.id, expired_delta=token_expires)
    return {"token": token}
